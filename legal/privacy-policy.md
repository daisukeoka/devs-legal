...
### 1.4 課金情報
本アプリのサブスクリプションおよびチケット課金は Apple（App Store）経由で処理されます。クレジットカード番号等の決済情報は Apple が直接管理しており、当社はこれを一切取得・保持しません。

サブスクリプション状態の管理および購入レシートの検証には RevenueCat を使用します。RevenueCat には、Apple が発行する匿名の購入レシートおよび当社が生成する匿名 ID（個人を特定しない）のみが送信されます。
...
Subscriptions and ticket purchases in the App are processed through Apple (App Store). Payment information such as credit card numbers is managed directly by Apple. We do not collect or retain any of this information.

We use RevenueCat to manage subscription status and verify purchase receipts. Only Apple-issued anonymous purchase receipts and an anonymous ID generated by us (which does not identify individual users) are sent to RevenueCat.
